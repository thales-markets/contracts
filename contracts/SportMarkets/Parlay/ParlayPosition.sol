...
contract ParlayPosition is IERC20 {
    /* ========== STATE VARIABLES ========== */
    ...
    address public thalesParlayAMM;
    /* ========== CONSTRUCTOR ========== */
    ...
    function initialize(
        ...
        address _thalesParlayAMM
    ) external {
        require(!initialized, "Parlay Market already initialized");
        initialized = true;
        parlayMarket = ParlayMarket(market);
        name = _name;
        symbol = _symbol;
        thalesParlayAMM = _thalesParlayAMM;
    }

    function allowance(address owner, address spender) external view override returns (uint256) {
        if (spender == thalesParlayAMM) {
    ...
    function burn(address claimant, uint amount) external onlyParlayMarket {
    ...
    function mint(address minter, uint amount) external onlyParlayMarket {
    ...
    modifier onlyParlayMarket {
        require(msg.sender == address(parlayMarket), "only the Parlay Market may perform these methods");
        _;
    }
