...
contract('ThalesRoyalePrivateRoom', accounts => {
	...
	beforeEach(async () => {
		...
		await ThalesRoyalePrivateRoomDeployed.upgradeToAndCall(
			ThalesRoyalePrivateRoomImplementation.address,
	...
	describe('ThalesRoyalePrivateRoom', () => {
		it('Sign up before creation of room', async () => {
			await expect(royale.signUpForRoom(1, { from: first })).to.be.revertedWith(
				'Room deleted or not published yet'
			);
		});
		...
		it('Create OPEN room and check values', async () => {
			const tx = await royale.createOpenRoom(
				SNX,
				GameType.LAST_MAN_STANDING,
				buyIn100,
				10,
		...
		it('Create CLOSED room and check values', async () => {
			const tx = await royale.createClosedRoom(
				SNX,
				GameType.LAST_MAN_STANDING,
				allowedPlayers,
				buyIn100,
		...
		it('Create OPEN room and check values and sign up period has expired', async () => {
			...
			await expect(royale.signUpForRoom(1, { from: third })).to.be.revertedWith(
		...
		it('Create OPEN room and check values and double sign up', async () => {
			...
			await expect(royale.signUpForRoom(1, { from: second })).to.be.revertedWith(
				'Player already signed up, for this room.'
			);
		});
		...
		it('Create CLOSED room and check values and check if player is not allowed', async () => {
			...
			await expect(royale.signUpForRoom(roomNumberCounter, { from: fourth })).to.be.revertedWith(
				'Can not sign up for room, not allowed or it is full'
			);
		});

		it('Create CLOSED room, sign up with success and check values', async () => {
			await royale.createClosedRoom(
				SNX,
				GameType.LAST_MAN_STANDING,
				allowedPlayers,
				buyIn100,
		...
		it('Start royale sign piriod is ongoing', async () => {
			...
			await expect(
				royale.startRoyaleInRoom(roomNumberCounter, { from: second })
			).to.be.revertedWith('Can not start until signup period expires for that room');
		...
		it('Start royale if user is not participant', async () => {
			...
			await expect(
				royale.startRoyaleInRoom(roomNumberCounter, { from: fourth })
			).to.be.revertedWith('You are not room participant');
		...
		it('Start royale already started', async () => {
			...
			await expect(
				royale.startRoyaleInRoom(roomNumberCounter, { from: second })
			).to.be.revertedWith('Royale already started for that room');
		});
		...
		it('Start royale and take a position which is not 1 or 2', async () => {
			...
			await expect(
				royale.takeAPositionInRoom(roomNumberCounter, 0, { from: second })
			).to.be.revertedWith('Position can only be 1 or 2');

			await expect(
				royale.takeAPositionInRoom(roomNumberCounter, 3, { from: second })
			).to.be.revertedWith('Position can only be 1 or 2');
		});
		...
		it('Take a position royale not started', async () => {
			...
			await expect(
				royale.takeAPositionInRoom(roomNumberCounter, 1, { from: second })
			).to.be.revertedWith('Competition not started yet');
		});
		...
		it('Take a position royale finished', async () => {
			...
			await expect(
				royale.takeAPositionInRoom(roomNumberCounter, 1, { from: second })
			).to.be.revertedWith('Competition finished');
			await expect(
				royale.takeAPositionInRoom(roomNumberCounter, 1, { from: first })
			).to.be.revertedWith('Competition finished');
		});
		...
		it('Take a position player did not sign up, not participant', async () => {
			...
			await expect(
				royale.takeAPositionInRoom(roomNumberCounter, 1, { from: third })
			).to.be.revertedWith('You are not room participant');

			await expect(
				royale.takeAPositionInRoom(roomNumberCounter, 2, { from: fourth })
			).to.be.revertedWith('You are not room participant');
		});
		...
		it('Same user take same position twice', async () => {
			...
			await expect(
				royale.takeAPositionInRoom(roomNumberCounter, 1, { from: second })
			).to.be.revertedWith('Same position');
		});
		...
		it('Take a position with royale which position time ends', async () => {
			...
			await expect(
				royale.takeAPositionInRoom(roomNumberCounter, 1, { from: second })
			).to.be.revertedWith('Round positioning finished');
		});
		...
		it('Close round competition not started', async () => {
			...
			await expect(
				royale.closeRoundInARoom(roomNumberCounter, { from: second })
			).to.be.revertedWith('Competition not started yet');
		});
		...
		it('Close round player that are not participat', async () => {
			...
			await expect(
				royale.closeRoundInARoom(roomNumberCounter, { from: fourth })
			).to.be.revertedWith('You are not room participant');
		});
		...
		it('Close round round still ongoing', async () => {
			...
			await expect(
				royale.closeRoundInARoom(roomNumberCounter, { from: second })
			).to.be.revertedWith('Can not close round yet');
		});
		...
		it('Win at last man standing, all lose, continue to last man standing', async () => {
			...
			await expect(
				royale.takeAPositionInRoom(roomNumberCounter, 1, { from: third })
			).to.be.revertedWith('Player no longer alive');
		...
		it('Win at limited number of rounds one player finished before each', async () => {
			await royale.createOpenRoom(
				SNX,
				GameType.LIMITED_NUMBER_OF_ROUNDS,
				buyIn100,
				3,
				1 * HOUR,
				3,
		...
		it('Two players win and one claim one expired and check require statements', async () => {
			...
			await expect(
				royale.claimRewardForRoom(roomNumberCounter, { from: first })
			...
			await expect(
				royale.claimRewardForRoom(roomNumberCounter, { from: third })
			).to.be.revertedWith('Player is not alive');
			...
			await expect(
				royale.claimRewardForRoom(roomNumberCounter, { from: first })
			).to.be.revertedWith('Player already collected reward');
		});
	});
	describe('Room management', () => {
		it('Try to set buy in, and buy in amount checking', async () => {
			...
			await expect(
				royale.setBuyInAmount(roomNumberCounter, buyInZero, { from: second })
			).to.be.revertedWith('You are not owner of room.');
			await expect(
				royale.setBuyInAmount(roomNumberCounter, buyInZero, { from: first })
			).to.be.revertedWith('Buy in must be greather then minimum');
			await expect(
				royale.setBuyInAmount(roomNumberCounter, buyIn100, { from: first })
			).to.be.revertedWith('Same amount');

			await royale.signUpForRoom(roomNumberCounter, { from: second });

			await expect(
				royale.setBuyInAmount(roomNumberCounter, buyIn99, { from: first })
			).to.be.revertedWith('Player already sign up for room, no change allowed');
			...
			await expect(
				royale.setBuyInAmount(roomNumberCounter, buyIn201, { from: first })
		...
		it('Set round length, and check values', async () => {
			...
			await expect(
				royale.setRoundLength(roomNumberCounter, 29, { from: second })
			).to.be.revertedWith('You are not owner of room.');
			await expect(
				royale.setRoundLength(roomNumberCounter, 29, { from: first })
			).to.be.revertedWith('Round length lower then minimum');
			await expect(
				royale.setRoundLength(roomNumberCounter, 2 * HOUR - 46 * MINUTES, { from: first })
			).to.be.revertedWith('Offset lower then minimum');

			const tx_2 = await royale.setRoundLength(roomNumberCounter, 2 * HOUR + 15 * MINUTES, {
				from: first,
			});
			...
			await expect(
				royale.setRoundLength(roomNumberCounter, 2 * HOUR, { from: first })
			).to.be.revertedWith('Player already sign up for room, no change allowed');
		...
		it('Set sign up period, check values', async () => {
			...
			await expect(
				royale.setRoomSignUpPeriod(roomNumberCounter, 2 * HOUR, { from: second })
			).to.be.revertedWith('You are not owner of room.');
			await expect(
				royale.setRoomSignUpPeriod(roomNumberCounter, 14, { from: first })
			).to.be.revertedWith('Sign in period lower then minimum');
			...
			await expect(
				royale.setRoomSignUpPeriod(roomNumberCounter, 3 * DAY, { from: first })
			).to.be.revertedWith('Player already sign up for room, no change allowed');
		...
		it('Set number of rounds', async () => {
			...
			await expect(
				royale.setNumberOfRoundsInRoom(roomNumberCounter, 7, { from: second })
			).to.be.revertedWith('You are not owner of room.');
			await expect(
				royale.setNumberOfRoundsInRoom(roomNumberCounter, 1, { from: first })
			).to.be.revertedWith('Must be more then minimum');
			...
			await expect(
				royale.setNumberOfRoundsInRoom(roomNumberCounter, 7, { from: first })
			).to.be.revertedWith('Player already sign up for room, no change allowed');
		...
		it('Set round choosing length', async () => {
			...
			await expect(
				royale.setRoundChoosingLength(roomNumberCounter, 1.5 * HOUR, { from: second })
			).to.be.revertedWith('You are not owner of room.');
			await expect(
				royale.setRoundChoosingLength(roomNumberCounter, 14, { from: first })
			).to.be.revertedWith('Round chosing lower then minimum');
			await expect(
				royale.setRoundChoosingLength(roomNumberCounter, 1 * HOUR + 46 * MINUTES, { from: first })
			).to.be.revertedWith('Round length lower then minimum');

			const tx_2 = await royale.setRoundChoosingLength(roomNumberCounter, 1.5 * HOUR, {
				from: first,
			});
			...
			await expect(
				royale.setRoundChoosingLength(roomNumberCounter, 7, { from: first })
			).to.be.revertedWith('Player already sign up for room, no change allowed');
		...
		it('Set oracle key, check values', async () => {
			...
			await expect(
				royale.setOracleKey(roomNumberCounter, ETH, { from: second })
			).to.be.revertedWith('You are not owner of room.');
			await expect(
				royale.setOracleKey(roomNumberCounter, toBytes32('GGG'), { from: first })
			).to.be.revertedWith('Not allowed assets');
			...
			await expect(royale.setOracleKey(roomNumberCounter, SNX, { from: first })).to.be.revertedWith(
				'Player already sign up for room, no change allowed'
			);
		...
		it('Set allowed players in closed room, check values', async () => {
			...
			await expect(
				royale.setNewAllowedPlayersPerRoomClosedRoom(roomNumberCounter, allowedPlayers, {
					from: first,
				})
			).to.be.revertedWith('Room need to be closed and  allowed players not empty');
			...
			const tx_2 = await royale.setNewAllowedPlayersPerRoomClosedRoom(
				roomNumberCounter,
				allowedPlayersUpdate,
				{ from: first }
			);
			...
			await royale.signUpForRoom(roomNumberCounter, { from: fourth });
			await expect(royale.signUpForRoom(roomNumberCounter, { from: second })).to.be.revertedWith(
				'Can not sign up for room, not allowed or it is full'
			);

			await expect(
				royale.setNewAllowedPlayersPerRoomClosedRoom(roomNumberCounter, allowedPlayersForAdding, {
					from: first,
				})
			).to.be.revertedWith('Player already sign up for room, no change allowed');
		...
		it('Add allowed players in closed room, take position with added player', async () => {
			...
			await expect(
				royale.addAllowedPlayerPerRoomClosedRoom(roomNumberCounter, third, { from: first })
			).to.be.revertedWith('Type of room needs to be closed');
			...
			const tx_2 = await royale.addAllowedPlayerPerRoomClosedRoom(roomNumberCounter, fourth, {
				from: first,
			});

			await expect(
				royale.addAllowedPlayerPerRoomClosedRoom(roomNumberCounter, fourth, { from: first })
		...
		it('Set amount of players, check values', async () => {
			...
			await expect(
				royale.setAmuontOfPlayersInOpenRoom(roomNumberCounter, 7, { from: second })
			).to.be.revertedWith('You are not owner of room.');
			await expect(
				royale.setAmuontOfPlayersInOpenRoom(roomNumberCounter, 1, { from: first })
			).to.be.revertedWith('Must be more then one player and open room');
			...
			await expect(
				royale.setAmuontOfPlayersInOpenRoom(roomNumberCounter, 7, { from: first })
			).to.be.revertedWith('Player already sign up for room, no change allowed');
		...
		it('Delete room', async () => {
			...
			await expect(royale.deleteRoom(roomNumberCounter, { from: second })).to.be.revertedWith(
				'You are not owner of room.'
			);
			...
			await expect(
				royale.setAmuontOfPlayersInOpenRoom(roomNumberCounter, 7, { from: first })
			).to.be.revertedWith('Deleted room');
			...
			await expect(royale.deleteRoom(roomNumberCounter, { from: first })).to.be.revertedWith(
				'Player already sign up for room, no change allowed'
			);
