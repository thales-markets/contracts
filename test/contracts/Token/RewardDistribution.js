	...
	describe('Functions', async () => {
		...
		describe('Vesting Writes', async () => {
			it('should revert if there is not enough THALES in the contracts balance', async () => {
				const REVERT =
					'VM Exception while processing transaction: revert Must be enough balance in the contract to provide for the reward distribution';
				const recipients = [beneficiary, ...new Array(499).fill(ZERO_ADDRESS)];
				const amounts = new Array(500).fill(web3.utils.toWei('1000'));

				await RewardDistribution.addTokens(web3.utils.toWei('100'));

				await assert.revert(RewardDistribution.fund(recipients, amounts), REVERT);
			});
		});
		...
		describe('Vesting Reads', async () => {
			beforeEach(async () => {
				...
				await RewardDistribution.addTokens(web3.utils.toWei('100'));
				await RewardDistribution.fund(recipients, [
					web3.utils.toWei('100'),
					...new Array(499).fill(0),
				]);
		...
		describe('Vesting', async () => {
			...
			it('should claim and update totalEscrowedAccountBalance', async () => {
				...
				let escrowedAccountBalance = await RewardDistribution.totalEscrowedAccountBalance(
					beneficiary
				);
			...
			it('should claim and update totalVestedAccountBalance', async () => {
				...
				let totalVestedAccountBalance = await RewardDistribution.totalVestedAccountBalance(
					beneficiary
				);
