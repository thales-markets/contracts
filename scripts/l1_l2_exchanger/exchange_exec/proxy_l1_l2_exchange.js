...
const { fromUnit } = require('../../../test/utils/index')();
...
async function main() {
	...
	const L2StandardBridge = new ethers.ContractFactory(
		L2StandardBridgeArtifacts.abi,
		L2StandardBridgeArtifacts.bytecode
	);
	const L1StandardBridge = new ethers.ContractFactory(
		L1StandardBridgeArtifacts.abi,
		L1StandardBridgeArtifacts.bytecode
	);
	...
	const OwnedUpgradeabilityProxy_deployed = await OwnedUpgradeabilityProxy.connect(l1Wallet).attach(
		OwnedUpgradeabilityProxyAddress
	);
	...
	const ProxyThalesExchanger_deployed = await ProxyThalesExchanger.connect(l1Wallet2).attach(
		OwnedUpgradeabilityProxyAddress
	);
	console.log('Thales ProxyExchanger on Kovan at: ', ProxyThalesExchanger_deployed.address);

	const ThalesExchanger_deployed = await ThalesExchanger.connect(l1Wallet2).attach(
		ThalesExchangerAddress
	);
	...
	const L2StandardBridge_USER_deployed = await L2StandardBridge.connect(l2Wallet2).attach(
		L2_BRIDGE_ADDRESS
	);
	const L2StandardBridge_deployed = await L2StandardBridge.connect(l2Wallet).attach(
		L2_BRIDGE_ADDRESS
	);
	...
	const L1StandardBridge_deployed = await L1StandardBridge.connect(l1Wallet).attach(
	...
	console.log(
		'L1 balance of Exchanger',
		ProxyThalesExchanger_deployed.address,
		':',
		fromUnit(balance.toString())
	);

	if (parseInt(fromUnit(balance.toString())) == 0) {
		console.log('\nTransferring ', TRANSFER_ERC20, ' THALES to Exchanger using proxy');
		let transfer = await OP_Thales_L1_deployed.transfer(
			ProxyThalesExchanger_deployed.address,
	...
	let approved = await OP_Thales_L1_deployed.allowance(
		ProxyThalesExchanger_deployed.address,
	...
	approved = await Thales_USER_deployed.allowance(
		l1Wallet2.address,
		ProxyThalesExchanger_deployed.address
	);
	...
	if (parseInt(fromUnit(approved.toString())) == 0) {
		...
		const tx1 = await Thales_USER_deployed.approve(
			ProxyThalesExchanger_deployed.address,
			w3utils.toWei(TRANSFER_ERC20)
		);
		await tx1.wait();
		...
		while (true) {
			try {
				...
				if (err.message.includes('execution failed due to an exception')) {
					...
					await delay(5000);
				} else if (err.message.includes('another transaction with same nonce in the queue')) {
					// this happens when we run tests in parallel
					await delay(5000);
