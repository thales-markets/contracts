...
contract('StakingThales', accounts => {
	...
	describe('Staking:', () => {
		...
		it('Single user: stake '+userStake+', ' +weeksOfStakingToTest+ ' weeks of claiming, fixed reward: '+fixedReward, async () => {
			let period = 0;
			...
			await ThalesDeployed.transfer(StakingThalesDeployed.address, (fixedReward*weeksOfStakingToTest), {
				from: owner,
			});
		...
		it('Single user: stake '+userStake+', ' +weeksOfStakingToTest+ ' weeks of claiming, fixed reward: '+fixedReward+', ustakes: '+partialUnstake, async () => {
			let period = 0;
		...
		it(users.length + ' users: stakes ['+stakes+'], ' +weeksOfStakingToTest+ ' weeks of claiming, fixed reward: '+fixedReward+', ustakes: ['+partialUnstakes+']', async () => {
			let period = 0;
			...
			for(let i=0; i<users.length; i++) {
				...
				assert.equal(answer, 0);
				await ThalesDeployed.approve(StakingThalesDeployed.address, stakes[i], { from: users[i] });
			...
			for(let i=0; i<users.length; i++) {
				...
				answer = await StakingThalesDeployed.getContractRewardFunds.call({ from: owner });
				assert.equal(web3.utils.toDecimal(answer), (totalStaked+(fixedReward*weeksOfStakingToTest)));
			...
			while(period < weeksOfStakingToTest) {
				...
				for(let i=0; i<users.length; i++) {
					...
					let portion = ((stakes[i]/totalStaked)+(web3.utils.toDecimal(stakedEscrowedBalances[i])/totalEscrowed))/2;
					if(totalEscrowed==0) {
					...
					assert.equal(web3.utils.toDecimal(stakedEscrowedBalances[i]), web3.utils.toDecimal(answer2));
					// the claimed rewards from period are compared with StakedEscrowedRewards in period+1
