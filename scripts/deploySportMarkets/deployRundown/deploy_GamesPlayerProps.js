...
async function main() {
	...
	let GamesPlayerProps = await ethers.getContractFactory('GamesPlayerProps');
	const playerProps = await upgrades.deployProxy(GamesPlayerProps, [
		owner.address,
		consumerAddress,
		verifierAddress,
		sportsManagerAddress,
		owner.address, // this is "dummy" need replacement after player props reciever deploy
		supportedSportIds,
	]);

	await playerProps.deployed();

	console.log('GamesPlayerProps deployed to:', playerProps.address);
	setTargetAddress('GamesPlayerProps', network, playerProps.address);

	const implementation = await getImplementationAddress(ethers.provider, playerProps.address);
