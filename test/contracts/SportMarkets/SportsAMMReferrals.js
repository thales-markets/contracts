...
contract('SportsAMM', (accounts) => {
	...
	describe('Testing referrals', () => {
		...
		it('Buy with referrals', async () => {
			let additionalSlippage = toUnit(0.2);
			let buyFromAmmQuote = await SportsAMM.buyFromAmmQuote(
				deployedMarket.address,
				position,
				toUnit(value)
			...
			await SportsAMM.buyFromAMMWithReferrer(
				deployedMarket.address,
				position,
				toUnit(value),
				buyFromAmmQuote,
		...
		it('Multi-collateral buy from AMM using referrals', async () => {
			...
			console.log('Balance of sUSD for first referrer: ', fromUnit(balanceOfFirstReferrer));
			console.log('Balance of sUSD for second referrer: ', fromUnit(balanceOfSecondReferrer));
			...
			console.log(
				'========================== 1st buyFromAMMWithDifferentCollateralAndReferrer ==============================='
			);
			...
			balanceOfSecondReferrer = await Thales.balanceOf(third);
			console.log(
				'========================== 2nd buyFromAMMWithDifferentCollateralAndReferrer ==============================='
			);
			await SportsAMM.buyFromAMMWithDifferentCollateralAndReferrer(
				deployedMarket.address,
				position,
				toUnit(value),
				toUnit((buyFromAmmQuoteUSDCCollateral / 1e6) * 0.9),
				additionalSlippage,
				testUSDC.address,
				third,
				{ from: first }
