...
contract('ThalesVault', accounts => {
	...
	describe('Initial properties check', () => {
		...
		it('should set first round to 1', async () => {
			assert.equal(await vault.round(), 1);
		});
	});
	...
	describe('Deposit', () => {
		it('should not deposit if no sUSD', async () => {
			...
			await assert.revert(vault.deposit(toUnit(100), { from: exersicer }), REVERT);
		});
		...
		it('should deposit before round starts', async () => {
