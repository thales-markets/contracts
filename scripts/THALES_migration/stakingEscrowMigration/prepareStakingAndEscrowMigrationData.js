...
async function prepareStakingEscrowMigration() {
	...
	for (let i = 0; i < addToEscrowEvents.length; ++i) {
		...
		if (!walletsforMigrationMap.has(escrowerAddress)) {
			...
			let walletObject = {};
			walletObject.totalEscrowed = numberExponentToLarge(escrowedBalanceOfBN.toString());
	...
	fs.writeFileSync(
		'scripts/THALES_migration/stakingEscrowMigration/migrationSnapshot.json',
	...
	const csv = new ObjectsToCsv(migrationWallets);
