...
async function main() {
	...
	const MockStaking = await ethers.getContractFactory('MockStakingThales');

	const MockStakingDeployed = await MockStaking.deploy();
	await MockStakingDeployed.deployed();

	console.log('MockStaking Deployed on', MockStakingDeployed.address);
	setTargetAddress('MockStaking', network, MockStakingDeployed.address);
	await delay(10000);

	try {
		await hre.run('verify:verify', {
			address: MockStakingDeployed.address,
		});
	} catch (e) {
		console.log(e);
	}
